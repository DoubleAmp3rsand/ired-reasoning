...
def main(argv=None):
    ...
    print(f"[1/4] loading autoencoder ({args.model}) ...")
    ae = FrozenT5Autoencoder(model_name=args.model, k=args.k).to(device)
    ae.eval()
    ...
    assert z_q.shape == (len(qs), args.k, ae.d_model)
    assert z_a.shape == (len(ans), args.k, ae.d_model)

    print("[3/4] building diffusion + EBM ...")
    ebm = EnergyTransformer(
        d_model=ae.d_model, k=args.k, n_layers=args.ebm_layers,
    ).to(device)
